...
// 用法: import { t } from '../i18n/useT';
//       <span>{t('app.foo.title', '原中文')}</span>
//       支持 i18next 插值: t('app.greet', '你好 {{name}}', { name: 'Tom' })
//      如果 zh.json 有 'app.foo.title' → 显示翻译
//      如果没有 → 显示 '原中文' (i18next 会原样返回 key, useT 判 key===value 时返回 defaultText)
//
// 重要: t 是纯函数,不是 React hook,可在 module 顶层调用。
// 之前的 useT (react-i18next 的 useTranslation) 必须在组件内调用,
// 而所有 .tsx 文件都错误地写成了 `const t = useT();` 在 module 顶层,
// 违反 Rules of Hooks → TranslationContext 还未挂载 → useContext 抛错 → 整个 React 树 crash
...
// 纯函数, 不依赖 React context, 可在 module 顶层 import 并调用
export const t: TFunction = (key, defaultText, vars) => {
  // 1. 查 zh.json / vi.json 翻译
  let v: string
  try {
    v = vars ? (i18n.t(key, vars) as string) : i18n.t(key)
  } catch {
