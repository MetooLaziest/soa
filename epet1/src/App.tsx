...
function ChatPage({ onClose }: { onClose: () => void }) {
  ...
  const doSend = async (message?: string, touchArea?: string) => {
    ...
    let userBubble = '';
    if (touchArea) {
      userBubble = `🤚 ${TOUCH_AREA_LABELS[touchArea] || t('app.chat.touch.fallback', '摸了摸')}`;
      // 点击反馈
      setPetPulse(true);
    ...
    try {
      ...
      setMessages((m) => [...m, {
        role: 'assistant',
        content: e.message?.includes('503') ? t('app.chat.error.noAI', '（AI 服务未配置，对话功能暂不可用）') : t('app.chat.error.network', '（网络出错了...）'),
      }]);
    } finally {
      setSending(false);
    }
  ...
  const handleTravel = async () => {
    ...
    if (!yardPets.some((p) => p.id === pet.id)) {
      alert(t('app.chat.alert.traveling', '宠物正在旅行中，无法再次派遣'));
      return;
    }
    // 跳转到旅行 Modal（带预选宠物）
    ...
    >
      ...
      <div className="chat-topbar">
        ...
        <button
          ...
          disabled={traveling}
        >
          {t('app.chat.button.travel', '✈️ 派遣旅行')}
        </button>
      </div>
      ...
      <div className="chat-input-bar">
        <input
          ...
          placeholder={t('app.chat.placeholder', '和宠物说点什么...')}
          disabled={sending}
        />
        <button
      ...
      <div style={{
        ...
        {wavePreview.wave?.batch_code && (
          <p style={{ fontSize: 13, color: '#94a3b8', margin: '0 0 4px' }}>
            {t('app.wave.batch', '批次: {code}', { code: wavePreview.wave.batch_code })}
          </p>
        )}
        {status && (
          <p style={{ fontSize: 13, color: '#94a3b8', margin: '0 0 20px' }}>
            {t('app.wave.currentStatus', '当前状态: {status}', { status: statusLabel[status] || status })}
          </p>
        )}
...
export default function App() {
      ...
      try {
        ...
        if (!travel && prevTravelRef.current) {
          ...
          if (returnedPet && returnedPet.growth_level > oldLevel) {
            showLevelUp(oldLevel, returnedPet.growth_level, returnedPet.nickname || t('app.pet.defaultName', '宠物'), returnedPet.pet_model_id);
          }
          prevTravelRef.current = null;
        }
      } catch (e) {
    ...
    const doClaim = async () => {
      try {
        ...
        if (result.merged && result.growth?.leveled_up) {
          showLevelUp(result.growth.growth_level - 1, result.growth.new_level, pet.nickname || t('app.pet.defaultName', '宠物'), pet.pet_model_id);
          setClaimResult({ ok: true, msg: t('app.collection.toast.mergedLevelUp', '🔄 重复宠物，已合并喂经验 +100！升级了！') });
        } else if (result.merged) {
          setClaimResult({ ok: true, msg: t('app.collection.toast.merged', '🔄 重复宠物，已合并喂经验 +100') });
        } else {
          setClaimResult({ ok: true, msg: t('app.collection.toast.claimed', '🎉 成功认领 {nickname}！', { nickname: pet.nickname || t('app.pet.defaultName', '宠物') }) });
        }
        // 刷新宠物列表
        const uid = authUserId;
        ...
        const msg = err?.response?.data?.error || err?.message || t('app.collection.toast.claimFailed', '认领失败');
  ...
  if (authLoading) {
    return (
      <div className="app-loading">
        ...
        <div>{t('app.nfcDialog.authenticating', '认证中...')}</div>
      </div>
    );
  }
  ...
  if (wavePreviewLoading) {
    return (
      <div className="app-loading">
        <div className="app-loading-spinner">🌀</div>
        <div>{t('app.nfcDialog.codeVerifying', '激活码校验中...')}</div>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="app-loading">
        <div className="app-loading-spinner">🌀</div>
        <div>{t('app.nfcDialog.loading', '加载中...')}</div>
      </div>
    );
  }
  ...
  return (
    <div className="app">
      ...
      <div className="mode-switch-bar">
        ...
        >{t('app.mode.yard', '🏡 庭院')}</button>
        <button
          className={`mode-switch-btn${homeMode === 'live' ? ' active' : ''}`}
          onClick={() => handleSwitchHomeMode('live')}
        >{t('app.mode.live', '📺 Live')}</button>
        {homeMode === 'yard' && (
          <span className="mode-switch-emotion">{t('app.mode.emotionPoints', '💛 {points}', { points: emotionPoints })}</span>
        )}
      </div>
        ...
        <NFCDialog
          ...
          onConfirm={async (nfcId: string) => {
            ...
            try {
              ...
              alert(result.merged ? t('app.nfcDialog.toast.merged', '🔄 重复宠物，已合并喂经验！') : t('app.nfcDialog.toast.activated', '🎉 激活成功！{nickname} 加入藏品库！', { nickname: pet.nickname }));
              setActiveModal('collection');
            } catch (e: any) {
              alert(e.message || t('app.nfcDialog.toast.activateFailed', '激活失败'));
            }
          }}
        />
      )}
            ...
            if (shopItemId && userId) {
              buyItem(userId, shopItemId)
                .then(res => {
                  ...
                  alert(t('app.match3.toast.passedAndBought', '🎉 通关！购买成功！'));
                })
                .catch(e => alert(e.message || t('app.shop.alert.purchaseFailed', '购买失败')));
            }
        ...
        <div style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: '#333', marginBottom: 4 }}>
              {t('app.levelUp.unlockTitle', '{name} 升级解锁!', { name: unlockPromptData.petName })}
            </div>
            ...
            <div style={{ textAlign: 'left', fontSize: 14, color: '#555', lineHeight: 2 }}>
              {unlockPromptData.config.shop_items && unlockPromptData.config.shop_items.length > 0 && (
                <div>{t('app.levelUp.unlock.shop', '🛒 解锁新商品 {count} 件', { count: unlockPromptData.config.shop_items.length })}</div>
              )}
              {unlockPromptData.config.dialog_topics && unlockPromptData.config.dialog_topics.length > 0 && (
                <div>{t('app.levelUp.unlock.topic', '💬 解锁新话题 {count} 个', { count: unlockPromptData.config.dialog_topics.length })}</div>
              )}
              {unlockPromptData.config.videos && unlockPromptData.config.videos.length > 0 && (
                <div>{t('app.levelUp.unlock.video', '🎬 解锁新视频 {count} 个', { count: unlockPromptData.config.videos.length })}</div>
              )}
            </div>
      ...
      {showPetActionOverlay && tappedPetPos && chatPetId && (() => {
        ...
        return createPortal(
          <PetActionOverlay
            petId={pet.id}
            petName={pet.nickname || pet.name || t('app.pet.defaultName', '宠物')}
