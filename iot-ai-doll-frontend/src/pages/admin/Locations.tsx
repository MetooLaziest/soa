/**
 * 地点管理 - CRUD 表格
 */
import { useState, useEffect } from 'react';
import client from '../../api/client';

interface Location { id: string; name: string; description: string; parent_id: string | null; created_at: string; }

export default function Locations() {
  const [locations, setLocations] = useState<Location[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: '', description: '' });
  const [editingId, setEditingId] = useState<string | null>(null);

  const load = async () => {
    setLoading(true);
    try {
      const res = await client.get('/db/locations');
      setLocations(res.data.locations || []);
    } finally { setLoading(false); }
  };

  useEffect(() => { load(); }, []);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (editingId) {
        await client.put(`/db/locations/${editingId}`, form);
      } else {
        await client.post('/db/locations', form);
      }
      setShowForm(false);
      setForm({ name: '', description: '' });
      setEditingId(null);
      load();
    } catch (err: any) {
      alert('操作失败: ' + (err.response?.data?.error || err.message));
    }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('确认删除该地点？')) return;
    await client.delete(`/db/locations/${id}`);
    load();
  };

  return (
    <div className="p-6 animate-fade-in-up">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-xl font-bold text-white">地点管理</h2>
        <button onClick={() => { setShowForm(true); setEditingId(null); setForm({ name: '', description: '' }); }}
          className="rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white hover:bg-purple-700"
        >+ 新增地点</button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 rounded-xl border border-white/10 bg-card p-5 space-y-4">
          <h3 className="font-semibold text-white">{editingId ? '编辑地点' : '新增地点'}</h3>
          <div>
            <label className="mb-1 block text-sm text-gray-300">名称</label>
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-purple-500"
              required />
          </div>
          <div>
            <label className="mb-1 block text-sm text-gray-300">描述</label>
            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
              className="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-purple-500"
              rows={3} />
          </div>
          <div className="flex gap-3">
            <button type="submit" className="rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white hover:bg-purple-700">保存</button>
            <button type="button" onClick={() => setShowForm(false)} className="rounded-lg bg-white/5 px-4 py-2 text-sm text-gray-300 hover:bg-white/10">取消</button>
          </div>
        </form>
      )}

      {loading ? <div className="text-gray-400">加载中...</div> : (
        <div className="overflow-x-auto rounded-xl border border-white/10 bg-card">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10 text-left text-xs text-gray-500 uppercase">
              <th className="px-4 py-3">名称</th><th className="px-4 py-3">描述</th><th className="px-4 py-3">操作</th>
            </tr></thead>
            <tbody>
              {locations.map(loc => (
                <tr key={loc.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="px-4 py-3 font-medium text-white">{loc.name}</td>
                  <td className="px-4 py-3 text-gray-400">{loc.description || '-'}</td>
                  <td className="px-4 py-3 space-x-3">
                    <button onClick={() => { setForm({ name: loc.name, description: loc.description }); setEditingId(loc.id); setShowForm(true); }}
                      className="text-cyan-400 hover:underline">编辑</button>
                    <button onClick={() => handleDelete(loc.id)}
                      className="text-red-400 hover:underline">删除</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {locations.length === 0 && <div className="py-8 text-center text-sm text-gray-500">暂无地点数据</div>}
        </div>
      )}
    </div>
  );
}
