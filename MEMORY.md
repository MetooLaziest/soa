...
- E-Pet AI对话功能已接入千问，后端 epet.js 聊天接口待改造以支持 temperature 参数和 RAG 知识库 L4 层
...
- E-Pet RAG 知识库管理和 Temperature 参数配置已全部完成（后端+前端 Admin2 页面）
- Admin2 后台已上线，部署于 /admin2/ 路径，支持宠物型号管理、RAG 知识库管理、Temperature 滑块配置
...
- RAG 知识库支持 M:N 关联，L4 层仅做选取，按需获取完整内容
- Temperature 默认 0.3（稳定），0-1 可调，滑步步进 0.1
- 研发规则已建立：本地备份→封板→版本控制→部署流程，GitHub 仓库 soa 已初始化推送
- RAG 触发采用混合方案：关键词匹配（快速）+ AI 判断（qwen-turbo）
- GitHub 仓库 soa 已初始化并推送，SSH 密钥认证
- Nginx 配置分散在 conf.d（HTTPS）和 sites-available（HTTP），两处需同步修改
