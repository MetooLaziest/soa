...
router.post('/', async (req, res) => {
  try {
    const { name, description, image_url, price_emotion, price_real, shop_tab } = req.body;
    if (!name) {
      return res.status(400).json({ success: false, error: 'name 必填' });
    }
    const r = await poolEpet1.query(
      `INSERT INTO shop_items (name, description, image_url, item_type, item_category, shop_tab, price_emotion, price_real)
       VALUES ($1, $2, $3, 'virtual', 'outfit', $4, $5, $6)
       RETURNING *`,
      [name, description || '', image_url || '', shop_tab || 'outfit', price_emotion || 0, price_real || 0]
    );
...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const fields = ['name', 'description', 'image_url', 'price_emotion', 'price_real', 'shop_tab', 'is_active'];
    const sets = [];
    const vals = [];
    let i = 1;
