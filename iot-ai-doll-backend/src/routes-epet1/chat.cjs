...
module.exports = (pool) => {
   ...
   */
  function buildSystemPromptV2(petModel, petInstance) {
    ...
    const ctxTemplate = petModel.context_memory_template || '';
    ...
    // ==== L4: 技能层 ====
    const l4 = skills || '暂无特殊技能。';

    // ==== L5: 上下文记忆层 ====
    let l5 = ctxTemplate
      .replace(/\{user_name\}/g, petInstance.user_name || '主人')
      .replace(/\{joy_value\}/g, petInstance.joy_value || '7')
      .replace(/\{anger_value\}/g, petInstance.anger_value || '1')
      .replace(/\{energy_value\}/g, petInstance.energy_value || '10')
      .replace(/\{emotion_status\}/g, petInstance.emotion_status || '平静')
      .replace(/\{recent_history\}/g, '（暂无近期互动记录）')
      .replace(/\{peer_history\}/g, '（暂无与其他玩偶的互动记录）');
